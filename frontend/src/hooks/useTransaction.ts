...
function friendlyError(e: unknown): string {
  ...
  if (/subject cannot vouch/i.test(m))
    return 'A subject cannot vouch for their own profile. Use a different wallet than the one that opened it';
...
export function useTransaction(onConfirmed?: () => void) {
      ...
      try {
        ...
        for (let i = 0; i < 5; i++) {
          try {
            ...
            const page = await fetchProfiles(
              Math.max(0, Math.floor(Math.max(0, stats.profiles - 1) / 20) * 20),
            );
            if (targetId) {
              result = page.find((p) => p.id === targetId) ?? null;
            } else {
  ...
  const submitOpen = useCallback(
    (address: `0x${string}`, handle: string, claim: string, onFlight?: (v: boolean) => void) =>
      run('open', address, (c) => openProfile(c, handle, claim), null, onFlight),
    [run],
  );

  const submitVouchTx = useCallback(
    (address: `0x${string}`, profileId: string, evidence: string, onFlight?: (v: boolean) => void) =>
      run('vouch', address, (c) => submitVouch(c, profileId, evidence), profileId, onFlight),
    [run],
  );

  return { state, submitOpen, submitVouchTx, reset };
}

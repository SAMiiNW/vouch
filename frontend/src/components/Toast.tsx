...
const ACCENT: Record<ToastKind, string> = {
  loading: 'text-peri-deep',
  success: 'text-trusted',
  error: 'text-unverified',
  info: 'text-ink',
};
...
function ToastRow({ item, onDismiss }: { item: ToastItem; onDismiss: () => void }) {
  ...
  return (
    <motion.div
      ...
      className="neu pointer-events-auto rounded-soft p-4"
    >
      <div className="flex items-start gap-3">
        <Icon size={18} className={`mt-0.5 shrink-0 ${ACCENT[item.kind]} ${item.kind === 'loading' ? 'animate-spin' : ''}`} />
        <div className="min-w-0 flex-1">
          ...
          {item.hash && (
            <a
              ...
              className="mt-1 flex items-center gap-1 font-mono text-xs text-ink-soft hover:text-peri-deep"
            >
        ...
        <button
          ...
          onClick={onDismiss}
          className="focus-ring shrink-0 text-ink-faint hover:text-ink"
        >
          <X size={16} />
        </button>
      </div>
